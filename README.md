...
1.  **Code as a Modality:** We treat source code not as plain text, but as a structured modality. By extracting and linearizing structural information (ASTs, CFGs, PDGs), we aim to create a more effective representation for large language models.
...
3.  **Retrieval-Augmented Generation for Code:** While RAG has proven effective for knowledge-intensive NLP tasks, its application to code summarization is still an emerging area. SP-RAG explores how to best retrieve relevant code snippets and structural information to augment the generation process.
4.  **Efficient Fine-Tuning:** The project leverages techniques like QLoRA (Quantized Low-Rank Adaptation) to efficiently fine-tune large language models (like Gemma) on the code summarization task, making it feasible to train powerful models on consumer-grade hardware.

## How It Works

The SP-RAG pipeline consists of the following stages:

1.  **Structural Feature Extraction:** Given a piece of code, we first extract its AST, CFG, and (in future work) PDG. These structures represent the code's syntactic, control-flow, and data-dependency properties, respectively.
